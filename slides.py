...
class SlideQubitBasic(ShorTalk):
    def construct(self):
...
class SlideQuantumSpeedup(ShorTalk):
    def construct(self):
...
class SlideGroverProblem(ShorTalk):
    def construct(self):
        setup = VGroup(
...
class SlideGroverOneIteration1(ShorTalk):
    def construct(self):
        # 1. Setup
        vals = [AMP_INIT] * 8
        chart, _, _ = self._grover_bars(vals, mark_idx=CHART_MARK)
...
class SlideGroverOneIteration2(ShorTalk):
    def construct(self):
        ...
        # 2. Define New State
        vals_new = [AMP_INIT] * 8
        vals_new[CHART_MARK] = -AMP_INIT
        chart_new, _, _ = self._grover_bars(vals_new, mark_idx=CHART_MARK)
        cap_new = th(T_STEP1, size=24, color=C_ACCENT).to_edge(UP, buff=HEADER_BUFF)
        bot_new = th(T_ORACLE_X, size=22, color=C_MUTED).move_to(bot)
...
class SlideGroverOneIteration3(ShorTalk):
    def construct(self):
        # 1. Match Previous End State
        vals_old = [AMP_INIT] * 8
...
class SlideGroverIterate(ShorTalk):
    def construct(self):
        a = 1 / np.sqrt(8)
...
class SlideFactoringKey(ShorTalk):
    def construct(self):
        setup = VGroup(
...
class SlideFactoringTrickExample(ShorTalk):
    def construct(self):
...
class SlideShorFindOrder(ShorTalk):
    def construct(self):
...
class SlideShor1_Parallel(ShorTalk):
    def construct(self):
        ...
        new_note = th(S_GATE, size=20, color=C_ACCENT).move_to(note)
        anims = [Transform(note, new_note)]
        ...
        for i in range(K_VALS):
            ...
            anims.append(Transform(f_texts[i], val))
            anims.append(
...
class SlideShor2_Collapse(ShorTalk):
    def construct(self):
        # Match previous state exactly
        k_strip, _, k_texts = self._register_strip(
            list(range(K_VALS)), center_y=1.5, label_text="k", color_fn=lambda i: WHITE
        )
        f_strip, f_cells, f_texts = self._register_strip(
            ...
            center_y=0.2,
            label_text="f(k)",
            color_fn=lambda i: PALETTE[F_PATTERN[i]],  # Use pattern colors
        )
        # Manually sync fill opacity for the 'add' state
        for cell in f_cells:
            cell.set_fill(opacity=0.3)

        note = th(S_GATE, size=20, color=C_ACCENT).next_to(k_strip, UP, buff=0.4)
        self.add(k_strip, f_strip, note)

        new_note = th(S_COLLAPSE, size=20, color=C_POS).move_to(note)
        keep_k = [3, 7, 11]
        fade_anims = [Transform(note, new_note)]
        ...
        for i in range(K_VALS):
            if i not in keep_k:
                ...
                fade_anims.append(k_texts[i].animate.set_color(C_POS).scale(1.2))
...
class SlideShor3_QFT(ShorTalk):
    def construct(self):
        # --- 1. RECONSTRUCT END OF SLIDE 2 (For Seamless Transition) ---
        # Note: Position must match Slide 2 exactly for the first frame
        note = th(S_COLLAPSE, size=20, color=C_POS).next_to(
            ORIGIN, UP, buff=2.3
        )  # Adjust to Slide 2's note position
        ...
        f_strip, f_cells, f_texts = self._register_strip(
            ...
            center_y=0.2,
            label_text="f(k)",
            color_fn=lambda i: PALETTE[F_PATTERN[i]],
        )

        # Apply the 'Collapsed' visual state immediately
        keep_k = [3, 7, 11]
        for i in range(K_VALS):
            if i not in keep_k:
                k_texts[i].set_opacity(0.1)
                f_cells[i].set_opacity(0.1)
                f_texts[i].set_opacity(0.1)
            else:
                k_texts[i].set_color(C_POS).scale(1.2)
                f_cells[i].set_fill(opacity=0.3)
        ...
        # Add all Slide 2 leftovers to the screen immediately
        self.add(k_strip,f_strip, r_arrows, note)

        # --- 2. PARAMETERS (N=15, x=2, r=4) ---
        r_val = 4
        Q = 16
        M_val = 3
        ...
        # --- 3. LATEX BLOCK (วางไว้ด้านบนเพื่อให้ไม่ทับกราฟ) ---
        # --- 3. LATEX BLOCK ---
        ...
        new_note = th(S_QFT, size=20, color=C_ACCENT).to_edge(UP, buff=1.0)
        
        formula_label = th("Probability Distribution (QFT Result):", size=18, color=C_ACCENT)
        
        # Position them relative to the note, but DO NOT group them yet
        prob_formula.next_to(new_note, DOWN, buff=0.8)
        formula_label.next_to(prob_formula, UP, buff=0.2)

        # --- 4. GRAPH & AXES ---
        # Fixed position: use shift or to_edge so it doesn't "look" at the formula_group
        ax = Axes(
            x_range=[0, 16.5, 4],
            y_range=[0, 1.2, 0.5],
            x_length=10,
        ...
        # --- 5. ANIMATION ---
        # Explicitly collect everything that needs to disappear
        fade_outs = [FadeOut(f_strip), FadeOut(r_arrows)]
        # fade_outs += [FadeOut(mob) for mob in k_texts]
        
        self.play(
            Transform(note, new_note),
            *fade_outs,
            # Morph directly into the standalone formula
            FadeTransform(k_strip, prob_formula),
            run_time=1.0,
        )

        self.play(
            FadeIn(formula_label, shift=UP*0.2),
            Create(ax), 
            run_time=1
        )
...
class SlideShorCaveats(ShorTalk):
    def construct(self):
        ...
        ).arrange(RIGHT, buff=0.2).to_edge(UP, buff=0.6)
        ...
        # Math inside boxes
        math_c = MathTex(r"O\left(e^{1.9 (\ln N)^{1/3} (\ln \ln N)^{2/3}}\right)", color=WHITE).scale(0.7).move_to(box_c)
...
# class SlideOrderFinding2(ShorTalk):
#     def construct(self):
#         # Left: concept
#         concept = VGroup(
...
#             ),
#             MathTex(r"a,\; a^2,\; a^3,\; \ldots \to 1", color=WHITE).scale(0.9),
#             VGroup(
#                 th("3)", size=22, color=C_MUTED),
#                 MathTex("r", color=C_ACCENT).scale(0.85),
#                 th("= จำนวนก้าวที่ใช้ (เรียกว่า order)", size=22, color=C_MUTED),
#             ).arrange(RIGHT, buff=0.15),
#             VGroup(
...
#             arr = CurvedArrow(
#                 a_pos, b_pos, color=C_POS, angle=-TAU / 6, tip_length=0.12
#             )
#             self.play(walker.animate.move_to(b_pos), Create(arr), run_time=0.5)
#             arrows.add(arr)
#         self.next_slide()
...
#             th("← ตรงกับรากพิเศษจากก่อนหน้า!", size=18, color=C_POS, weight=BOLD),
#         ).arrange(DOWN, buff=0.1)
#         result.next_to(clock, DOWN, buff=0.2)
#         self.play(FadeIn(result))
#         self.next_slide()
        
# class SlideShorParallel(ShorTalk):
#     def construct(self):
#         intro = VGroup(
#             th("เราอยากหา r ของ", size=24, color=C_MUTED),
#             MathTex(r"f(k) = 2^k \bmod 15", color=C_ACCENT).scale(0.9),
#         ).arrange(RIGHT, buff=0.25)
#         intro.to_edge(UP, buff=0.2)
...
#         sp_note = th(
#             "input register: superposition ของทุก k (amplitude เท่ากัน)",
#             size=20,
#             color=C_MUTED,
#         )
...
#             MathTex(
#                 r"|k\rangle |0\rangle \;\to\; |k\rangle |f(k)\rangle", color=C_ACCENT
#             ).scale(0.85),
#         ).arrange(RIGHT, buff=0.15)
...
#         ex_input = SurroundingRectangle(
#             input_cells[2], color=C_POS, buff=0.05, stroke_width=3
#         )
#         ex_output = SurroundingRectangle(
#             zero_cells[2], color=C_POS, buff=0.05, stroke_width=3
#         )
#         ex_label = MathTex(
#             r"|2\rangle|0\rangle \to |2\rangle|4\rangle", color=C_POS
#         ).scale(0.7)
...
#         self.play(
#             Transform(zero_texts[2], new_4),
#             zero_cells[2].animate.set_stroke(C_POS).set_fill(C_POS, opacity=0.3),
#         )
...
#         apply_note = th(
#             "เพราะ input อยู่ใน superposition → gate ครั้งเดียว" " ทำงานพร้อมกันทุก k",
#             size=20,
#             color=C_POS,
#         )
...
#         anims = []
#         for i in range(K):
#             new_text = MathTex(str(f_values[i]), color=C_ACCENT).scale(0.55)
#             new_text.move_to(zero_texts[i])
#             anims.append(Transform(zero_texts[i], new_text))
#             anims.append(
#                 zero_cells[i]
#                 .animate.set_stroke(C_ACCENT)
#                 .set_fill(C_ACCENT, opacity=0.15)
#             )
#         self.play(*anims)
#         self.next_slide()
...
#             *[
#                 out_cells[i]
#                 .animate.set_stroke(palette[f_values[i]])
#                 .set_fill(palette[f_values[i]], opacity=0.3)
#                 for i in range(K)
#             ]
#             + [out_texts[i].animate.set_color(palette[f_values[i]]) for i in range(K)]
#         )

#         pattern = th(
#             "สังเกต: f(k) วนซ้ำทุก 4 ตัว — คาบ r = 4", size=24, color=C_POS, weight=BOLD
#         )
...
#         self.play(
#             *(
#                 [self.shor_input_cells[i].animate.set_opacity(0.15) for i in drop_k]
#                 + [self.shor_input_texts[i].animate.set_opacity(0.15) for i in drop_k]
#                 + [self.shor_out_cells[i].animate.set_opacity(0.15) for i in drop_k]
#                 + [self.shor_out_texts[i].animate.set_opacity(0.15) for i in drop_k]
#                 + [
#                     self.shor_input_cells[i]
#                     .animate.set_stroke(C_POS)
#                     .set_fill(C_POS, opacity=0.35)
#                     for i in keep_k
#                 ]
#                 + [self.shor_input_texts[i].animate.set_color(C_POS) for i in keep_k]
#             )
#         )
#         self.next_slide()
...
#             br = DoubleArrow(
#                 a_pt, b_pt, color=C_ACCENT, stroke_width=2, tip_length=0.12, buff=0
#             )
#             mid = (a_pt + b_pt) / 2
...
#             th("— คาบถูกฝังอยู่ในระยะห่าง", size=24, color=C_POS, weight=BOLD),
#         ).arrange(RIGHT, buff=0.2)
...
#         analogy = th("เหมือน Fourier — แปลงคาบให้เป็น peak ที่ความถี่", size=20, color=C_MUTED)
...
#         )
#         time_label = th("Before: k เหลือ = {3, 7, 11, 15}", size=18, color=C_ACCENT)
...
#         for k in [3, 7, 11, 15]:
#             bar = Line(
#                 ax_time.c2p(k, 0), ax_time.c2p(k, 1.0), color=C_ACCENT, stroke_width=5
#             )
#             time_bars.add(bar)
#         self.play(LaggedStart(*[Create(b) for b in time_bars], lag_ratio=0.1))
#         self.next_slide()
...
#         freq_label = th(
#             "After: spike ที่ j = 0, 4, 8, 12  (ห่างกัน 2²ⁿ/r)", size=18, color=C_POS
#         )
...
#                 h = 0.05
#                 col = C_MUTED
#             bar = Line(ax_freq.c2p(j, 0), ax_freq.c2p(j, h), color=col, stroke_width=4)
#             freq_bars.add(bar)
#         self.play(
#             LaggedStart(*[Create(b) for b in freq_bars], lag_ratio=0.04),
#             FadeIn(freq_label),
#         )
#         self.next_slide()

#         # How to actually extract r — measure once + continued fraction
#         explain1 = th("วัดครั้งเดียว → ได้ค่า j สุ่มจากบรรดา spike", size=18, color=C_POS)

#         explain2 = VGroup(
#             MathTex(r"\tfrac{j}{2^{2n}} \approx \tfrac{c}{r}", color=C_ACCENT).scale(
#                 0.75
#             ),
#             th("→ continued fraction ดึง r ออกมา", size=18, color=C_ACCENT),
#         ).arrange(RIGHT, buff=0.2)

#         retry = th("ถ้าโชคไม่ดี (c กับ r มีตัวประกอบร่วม) → ลองใหม่", size=16, color=C_MUTED)

#         conclusion_block = VGroup(explain1, explain2, retry).arrange(DOWN, buff=0.1)
...
class SlideShorRecover(ShorTalk):
    def construct(self):
...
class SlideShorReality(ShorTalk):
    def construct(self):
